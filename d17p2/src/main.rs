...
impl Coordinates {
	...
	fn get_adjusted(&self, x_shift: i32, y_shift: i32, z_shift: i32, w_shift: i32) -> Self {
		let x = self.x + x_shift;
		let y = self.y + y_shift;
		let z = self.z + z_shift;
		let w = self.w + w_shift;
		Self { x, y, z, w }
	}
}
		...
		.map(|coords| {
			...
			for x_change in -1..=1 {
				for y_change in -1..=1 {
					for z_change in -1..=1 {
						for w_change in -1..=1 {
							all_coords.push(coords.get_adjusted(x_change, y_change, z_change, w_change));
						}
					}
				}
			}
	...
	for coords in coords_to_check.iter() {
		...
		for x_change in -1..=1 {
			for y_change in -1..=1 {
				for z_change in -1..=1 {
					for w_change in -1..=1 {
						if x_change == 0 && y_change == 0 && z_change == 0 && w_change == 0 {
							continue; // Omit the cube itself from its neighbors
						}
						let check_cube_active = start_cubes
							.get(&coords.get_adjusted(x_change, y_change, z_change, w_change))
							.unwrap_or(&false);
	...
	let mut cubes: HashMap<Coordinates, bool> = {
		...
		for (col_index, col) in cubes.split('\n').filter(|s| !s.is_empty()).enumerate() {
			for (row_index, row_val) in col.chars().enumerate() {
				cube_map.insert(
					Coordinates::new(row_index as i32, col_index as i32, 0, 0),
					row_val == '#',
				);
			}
		}
